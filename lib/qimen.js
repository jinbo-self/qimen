...
function getGanZhiIndex(ganZhi) {
  return SEXAGENARY.indexOf(ganZhi);
}
/**
 * 计算阴阳遁局数
...
function calculateJuShu(date, method = '时家') {
    ...
    let ganZhi = '';
    if (method === '时家') {
        const dayGanZhi = lunar.getDayInGanZhi(); // 时家奇门用日干支判断元,默认日干支
        const dayIndex = getGanZhiIndex(dayGanZhi);
        const fuTouIndex = dayIndex - (dayIndex % 5);
        const fuTouGanZhi = SEXAGENARY[fuTouIndex];
        ganZhi = fuTouGanZhi;
    } else if (method === '日家') {
    ...
    console.log("阴阳遁类型:", yinYangType);
    console.log("局数:", juNumber);
    console.log("上中下元:", ['上元', '中元', '下元'][yuan]);
    console.log("当前节气:", jieQiName);
    return {
        jieQiName: jieQiName,
        type: yinYangType,
        number: juNumber,
...
function distributeTianPan(diPan, xunShou, shiGan, type) {
    ...
    for (const gong in result) {
        //地盘5寄宿坤二宮，随坤二宮移动
        if (result[gong] === diPan['2']) {
            result[gong] = diPan['2'] + '、' + diPan['5'];
            break;
        }
    }
    return result;
            ...
            res.on('end', () => {
                try {
                    ...
                    if (result.choices && result.choices.length > 0) {
                        ...
                        reject(new Error('DeepSeek API response error'));
                    }
                } catch (e) {
                    reject(e);
                }
...
function formatPanData(qimenResult) {
    let panData = '';
    ...
    panData += `【奇门排盘信息】\n`;
    panData += `时间：${qimenResult.basicInfo.date || ''}\n`;
    panData += `农历：${qimenResult.basicInfo.lunarDate || ''}\n`;
    panData += `四柱：${qimenResult.siZhu ? `${qimenResult.siZhu.year}年 ${qimenResult.siZhu.month}月 ${qimenResult.siZhu.day}日 ${qimenResult.siZhu.time}时` : ''}\n`;
    panData += `局数：${qimenResult.juShu ? qimenResult.juShu.fullName : ''}\n`;
    panData += `值符：${qimenResult.zhiFuXing || ''} 落${gongNames[qimenResult.zhiFuGong] || ''}宫\n`;
    panData += `值使：${qimenResult.zhiShiMen || ''} 落${gongNames[qimenResult.zhiShiGong] || ''}宫\n`;
    panData += `空亡：${qimenResult.kongWangZhi ? qimenResult.kongWangZhi.join('、') : ''}（${qimenResult.kongWangGong ? qimenResult.kongWangGong.map(g => gongNames[g]).join('、') : ''}宫）\n`;
    panData += `驿马：${qimenResult.maStar ? `${qimenResult.maStar.zhi}(${gongNames[qimenResult.maStar.gong] || ''}宫)` : ''}\n`;
    panData += `\n【九宫详细分布】\n`;
    
    gongOrder.forEach(gong => {
        panData += `${gongNames[gong]}宫：\n`;
        panData += `  天盘干：${qimenResult.sanQiLiuYi ? qimenResult.sanQiLiuYi[gong] || '-' : '-'}\n`;
        panData += `  地盘干：${qimenResult.diPan ? qimenResult.diPan[gong] || '-' : '-'}\n`;
        panData += `  九星：${qimenResult.jiuXing ? qimenResult.jiuXing[gong] || '-' : '-'}\n`;
        panData += `  八门：${qimenResult.baMen ? qimenResult.baMen[gong] || '-' : '-'}\n`;
        panData += `  八神：${qimenResult.baShen ? qimenResult.baShen[gong] || '-' : '-'}\n`;
        panData += `  暗干：${qimenResult.anGan ? qimenResult.anGan[gong] || '-' : '-'}\n`;
        if (qimenResult.jiuGongAnalysis && qimenResult.jiuGongAnalysis[gong]) {
            panData += `  吉凶：${qimenResult.jiuGongAnalysis[gong].jiXiongText || '-'}\n`;
        }
        panData += `\n`;
    });
...
/**
 * 综合分析奇门盘（调用DeepSeek API）
 * @param {Object} qimenResult 完整的排盘结果
 * @param {String} purpose 排盘目的（综合、事业、财运、婚姻、健康、学业）
 * @param {String} question 用户问题
 * @returns {Object} 综合分析结果
 */
async function overallAnalysis(qimenResult, purpose = '综合', question = '') {
    const purposeMap = {
        '综合': 'general',
        '事业': 'career',
        '财运': 'wealth',
    ...
    const promptKey = purposeMap[purpose] || 'general';
    const promptConfig = prompts[promptKey];
    
    if (!promptConfig) {
        return {
            overallJiXiong: 'ping',
            overallJiXiongText: '平',
            bestGong: '',
            suggestions: ['无法获取分析结果']
        };
    }
    
    const panData = formatPanData(qimenResult);
    const userQuestion = question || '请分析当前时运状况';
    ...
    const prompt = [
        ...
        {
            role: 'user',
            content: promptConfig.user.replace('{panData}', panData).replace('{question}', userQuestion)
        }
    ];
    ...
    try {
        ...
        if (!apiKey) {
            return {
                ...
                overallJiXiongText: '平',
                bestGong: '',
                suggestions: ['请配置DeepSeek API密钥'],
                aiAnalysis: '请在环境变量中配置DEEPSEEK_API_KEY'
            };
        }
        ...
        const zhiFuGong = qimenResult.zhiFuGong;
        const zhiShiGong = qimenResult.zhiShiGong;
        const jiuGongAnalysis = qimenResult.jiuGongAnalysis;
        
        const zhiFuJiXiong = jiuGongAnalysis && jiuGongAnalysis[zhiFuGong] ? jiuGongAnalysis[zhiFuGong].jiXiong : 'ping';
        const zhiShiJiXiong = jiuGongAnalysis && jiuGongAnalysis[zhiShiGong] ? jiuGongAnalysis[zhiShiGong].jiXiong : 'ping';
        ...
        suggestions.push('(AI分析失败，使用本地分析)');
 ...
 * @returns {Object} 排盘结果
 */
async function calculate(date, options = {}) {
    ...
    try {
        ...
        console.log(opts.method,'时柱:',siZhu.time);
        console.log(opts.method,'旬首:',xunShou);
        ...
        console.log(opts.method,'落宫干/时干:',luoGongGan);
        
        // 获取空亡信息  --已核实，没问题
        const kongWangZhi = getKongWang(lunar, opts.method);
        const kongWangGong = getKongWangGong(kongWangZhi);
        console.log(opts.method,'空亡信息:',kongWangGong);
        // 获取驿马星信息
        const maStar = getMaStar(lunar, opts.method);
        console.log(opts.method,'驿马星信息:',maStar);
        
        // 1. 计算地盘干（根据局数固定排布）--已核实，没问题
        const diPan = diPanModule.getDiPan(juShu.type, parseInt(juShu.number));
        console.log(opts.method,'地盘干:',diPan);

        // 2. 计算天盘干（转盘排法）  --已核实，没问题,修改过，地盘中宫随坤二宫一起转动，天盘其中一个宫会有两个干
        const tianPan = distributeTianPan(diPan, xunShou, luoGongGan, juShu.type);
        console.log(opts.method,'天盘干:',tianPan);
        
        // 保留 sanQiLiuYi 为天盘干（兼容旧代码）
        const sanQiLiuYi = tianPan;

        // 排布九星（使用地盘干找值符宫，传入时干计算转动）  --已核实，没问题
        const jiuXingResult = jiuXingModule.distributeJiuXing(diPan, xunShou, luoGongGan, juShu.type);
        console.log(opts.method,'九星分布:',jiuXingResult);

        // 排布八门（使用地盘干找时干落宫，传入阴阳遁类型）  --已核实，没问题
        const baMenResult = baMenModule.distributeBaMen(jiuXingResult.zhiFuGong, lunar.getTimeXun()[1], siZhu.time[1], juShu.type);
        console.log(opts.method,'八门分布:',baMenResult);
        // 排布八神（从值符落宫开始，阳遁顺时针，阴遁逆时针）  --已核实，没问题
        ...
        // 综合分析（调用DeepSeek API）
        const analysis = await overallAnalysis(interimResult, opts.purpose, opts.question);

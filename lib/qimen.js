...
function formatPanData(qimenResult) {
    ...
    panData += `【奇门排盘信息】\n`;
    panData += `时间：${qimenResult.basicInfo ? qimenResult.basicInfo.date || '' : ''}\n`;
    panData += `农历：${qimenResult.basicInfo ? qimenResult.basicInfo.lunarDate || '' : ''}\n`;
...
async function aiAnalysis(qimenResult, purpose = '综合', question = '') {
    ...
    let panData;
    try {
        panData = formatPanData(qimenResult);
    } catch (e) {
        console.error('formatPanData错误:', e);
        panData = '排盘数据格式化失败';
    }
    ...
    const userPrompt = promptConfig.user.replace('{panData}', panData).replace('{question}', userQuestion);
    ...
    try {
        const apiKey = 'sk-70837417ce4a43d4b6de144fa79fb372';
        if (!apiKey) {
            return {
                aiAnalysis: '请配置DeepSeek API密钥'
            };
        }
...
async function calculate(date, options = {}) {
    ...
    try {
        ...
        let analysis = {
            overallJiXiong: 'ping',
            overallJiXiongText: '分析中...',
            bestGong: '',
            suggestions: ['正在进行AI分析，请稍候...'],
            aiAnalysis: ''
        };
        
        if (!opts.skipAnalysis) {
            analysis = overallAnalysis(jiuGongAnalysis, interimResult.zhiFuGong, interimResult.zhiShiGong, opts.purpose);
        }
